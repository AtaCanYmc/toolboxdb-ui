...
export async function sendChatMessage(payload: { message: string; history: { role: string; content: string }[] }) {
    const response = await fetch(`${BASE_URL}/suggestions/chat`, {
        method: 'POST',
        headers: getHeaders(),
...
export async function updateInvoiceItem(itemId: string, payload: {
    clean_name?: string;
    quantity?: number;
